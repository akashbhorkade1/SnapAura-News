...
### 4.7 Career Recruitment Articles (automated template v2)
Career posts follow the bilingual Gen Z / mobile-first brief, enforced by `scripts/generate-ai-drafts.js` (never per-article edits):
...
- **Required order:** At a Glance → "Can I Apply? 👀" (Qualification / Experience / Age / Other sub-cards + one neutral decision line) → "Why This Job? 🔥" (2–4 practical points, ONLY if the source supports them, never clickbait) → "Important Dates 📅" (timeline table, last date emphasized) → "Application Fee 💰" (category table, only source-listed categories) → "Selection Process 📝" (steps only if sourced, else "Selection process: Check the official notification.") → "Quick Eligibility Check ✅" (self-checklist, never claims eligibility) → "What should I do now? 🚀" (up to 5 practical steps, ending with saving confirmation details) → short Background only if genuinely useful (max ~80–100 words) → "मराठीत झटपट समजून घ्या 🇮🇳" → curated Important Links with calm CTA labels ("Notification & Online Application →") → source line → "More Career Updates" (max 3).
- **English style:** simple, modern, Indian-job-seeker friendly; short paragraphs; emoji only as section markers; source mentioned once (transparency comes from the source line, not repeated citations); gaps written as "To be announced" / "Not specified in the available source". CLARITY > LENGTH — complete but concise (a 2–4 minute read), never padded.
- **Marathi style:** a natural conversational QUICK GUIDE (3–4 sentences + 4–6 bullet micro-cards) for Marathi-speaking job seekers — NEVER a word-for-word translation; everyday Marathi mixed with common English words, no Sanskritized constructions.
...
- **Accuracy > engagement:** never invent dates, fees, vacancies, stages, salary, benefits, or URLs. `scripts/validate.js` enforces the required sections, link caps, and banned-CTA rules for every Career article.
