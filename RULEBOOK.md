...
> **Career automation note:** `scripts/generate-ai-drafts.js` encodes the Career template v2 (`CAREER_LINK_POLICY` + `CAREER_STYLE_GUIDE`) — bilingual English core + Marathi quick guide. Flow: hook → At-a-Glance card → "Can I Apply?" (eligibility sub-cards + decision line) → "Why This Job?" (source-backed only) → "Important Dates" timeline → "Application Fee" table → "Selection Process" → "Quick Eligibility Check" → "What should I do now?" (up to 5 steps) → concise Background only if useful → "मराठीत झटपट समजून घ्या 🇮🇳" (conversational quick guide with bullet micro-cards, NEVER a translation) → curated 1–3 official Important Links with calm CTA labels → source-transparency line → "More Career Updates" (max 3). The sanitizer strips chrome/social/tool links and banned marketing CTAs, rebuilds Important Links from curated source URLs, and warns when a v2 section is missing. `scripts/validate.js` enforces the required sections, link caps, and banned-CTA rules for every Career article.
...
### 4.7 Career Recruitment Articles (automated template v3)
Career posts follow the bilingual Gen Z / mobile-first brief (English complete core + FULL Marathi coverage), enforced by `scripts/generate-ai-drafts.js` (never per-article edits):
- **First screen:** user-focused headline + 1–2 sentence hook (what job, how many vacancies, who can apply, last date) + "At a Glance" card (Organization, Posts, Vacancies, Qualification, Age, Fee, Last Date, Location; "To be announced" when unknown, never guessed).
- **Required order:** At a Glance → "Can I Apply? 👀" (Qualification / Experience / Age / Other sub-cards + one neutral decision line) → "Why This Job? 🔥" (2–4 practical points, ONLY if the source supports them, never clickbait) → "Important Dates 📅" (timeline table, last date emphasized) → "Application Fee 💰" (category table, only source-listed categories) → "Selection Process 📝" (steps only if sourced, else "Selection process: Check the official notification.") → "Quick Eligibility Check ✅" (self-checklist, never claims eligibility) → "What should I do now? 🚀" (up to 5 practical steps, ending with saving confirmation details) → short Background only if genuinely useful (max ~80–100 words) → "मराठीत झटपट समजून घ्या 🇮🇳" (FULL Marathi coverage — see below) → curated Important Links with calm CTA labels ("Notification & Online Application →") → "More Career Updates" (max 3). Career pages carry NO separate "Source: … Original report" footer line.
- **English style:** simple, modern, Indian-job-seeker friendly; short paragraphs; emoji only as section markers; the reporting source is named once inside the body (Career pages carry no separate "Source: … Original report" footer line); gaps written as "To be announced" / "Not specified in the available source". CLARITY > LENGTH — complete but concise (a 2–4 minute read), never padded.
- **Marathi style:** FULL coverage, not a 3–4 sentence summary — the "मराठीत झटपट समजून घ्या 🇮🇳" section must mirror ALL the English content to the same depth (eligibility: qualification / experience / age; posts & vacancies; important dates; fee; selection; location; action steps), typically roughly half the English core's length. Natural, everyday conversational Marathi (never a word-for-word translation, never machine-translated feel), everyday words mixed with common English job terms, no Sanskritized constructions.
- **Important Links:** 1–3 official links only (notification, apply portal, official site). Never invent URLs; never add mock tests, calculators, tools, apps, Telegram/WhatsApp/social, linktree, promos, category pages, or unrelated articles. No aggressive CTAs ("CLICK HERE!!!", "APPLY NOW!!!").
...
### 4.8 Current Affairs Articles (automated template v3 FINAL — weekly+monthly, EN+MR, SEO-only exams)
Current Affairs posts follow the weekly/monthly competitive-exam revision brief, enforced by `scripts/generate-ai-drafts.js` (never per-article edits):
- **FREQUENCY (most important):** weekly + monthly ONLY — never daily. Daily material is collected internally and consolidated into the weekly/monthly roundup. Duplicate-period detection prevents two digests for the same week/month.
- **Two separated versions in one article:** complete natural ENGLISH article first, then a clearly separated complete natural MARATHI article (मराठीत — साप्ताहिक/मासिक चालू घडामोडी). Never mixed paragraphs, never sentence-by-sentence translation; Marathi reads as originally written for Marathi readers.
- **English body:** what happened, why it matters, key facts, background, data, institutional relevance; exam-neutral phrases only (important for competitive-exam preparation / exam-relevant facts / key facts to remember / important points for revision). NEVER repeat exam names (UPSC/MPSC/SSC/RRB/ZP/Police Bharti) in the body.
- **Marathi body:** independently readable natural Marathi (या आठवड्यातील महत्त्वाच्या घडामोडी / काय घडले / महत्त्वाचे तथ्य / पार्श्वभूमी / परीक्षेच्या दृष्टीने महत्त्वाचे मुद्दे / 1-मिनिट Revision / सराव प्रश्न); uses स्पर्धा परीक्षेच्या दृष्टीने महत्त्वाचे मुद्दे style wording, never repeated exam names.
- **Exam names = SEO metadata only:** curated keyword set (weekly/monthly current affairs 2026, UPSC/MPSC/SSC/RRB/railway/ZP bharti/police bharti/banking/IBPS/defence/competitive/government exam current affairs + Marathi equivalents) merged into meta keywords/news_keywords/schema — never a visible keyword paragraph, never stuffed into titles.
- **Weekly EN flow:** H1 "Weekly Current Affairs: [DATE RANGE]" + short intro → "📰 This Week at a Glance" table (Topic | What Happened | Key Point) → per-topic h2s using ONLY relevant sub-blocks (Why It Matters / What Happened / Key Facts / Background / Key Takeaway) → "🎯 Exam Focus" → "⚡ 1-Minute Revision" → "🧠 Quick Quiz" (verified facts only). Weekly MR follows as a separated article.
- **Monthly EN flow:** H1 "Monthly Current Affairs: [MONTH YEAR]" → "📰 Month at a Glance" → ONLY non-empty categories (National/International/Economy/Polity & Governance/Environment/Sci-Tech/Defence/Awards & Events/Sports/Reports) → "🎯 Monthly Exam Focus" → "⚡ Quick Revision" → "🧠 Monthly Quiz" → "🔗 Sources". Monthly MR follows with natural headings + स्रोत.
- **No forced single-exam structure:** universal headings (Exam Focus / Important Facts / Why It Matters / Quick Revision) — never "UPSC Connection / Mains Angle / Prelims Focus" on every article. Broad audience via content quality.
- **SOURCE-COMPLETENESS:** never fabricate; release-announcement without topics → skip or short honest notice. No "Important Links" farm — only "🔗 Sources" with relevant original/official sources.
- **Titles:** natural ("Weekly Current Affairs: 7–13 September 2026"), never exam-list stuffed.
- **Style:** Gen-Z/mobile-first, short paragraphs, tables, bullets, emoji section markers, quizzes. `scripts/validate.js` enforces v3 sections, exam-name limits, and filler bans. Career system untouched.
